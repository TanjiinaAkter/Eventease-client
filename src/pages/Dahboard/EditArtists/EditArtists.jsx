...
const EditArtists = () => {
  ...
  useEffect(() => {
    if (artists.length > 0) {
      const getArtist = artists.find((art) => art._id === id);
      setArtist(getArtist);
      if (getArtist) {
        setValue("artistname", getArtist.name);
        setValue("genre", getArtist.genre);
        setValue("contact", getArtist.phone);
        setValue("email", getArtist.email);
        setValue("instagram", getArtist.instagramlink);
        setValue("bio", getArtist.bio);
        setValue("photo", getArtist.photo);
        setValue("facebook", getArtist.facebooklink);
      }
    }
  }, [artists, id, setValue]);
        ...
        .then((res) => {
          ...
          if (res.data.modifiedCount === 1) {
            Swal.fire({
              position: "top-end",
              icon: "success",
              title: `${data.artistname} has been modified!!`,
        ...
        <div className="shadow-2xl bg-[#0f1c1c] p-6 rounded-none w-full lg:w-[80%] h-full">
          <form onSubmit={handleSubmit(onSubmit)} className="form space-y-3">
            ...
            <div className="grid md:w-full my-7">
              <button type="submit" className="button-style uppercase">
                Modify ARTIST
              </button>
            </div>
          </form>
